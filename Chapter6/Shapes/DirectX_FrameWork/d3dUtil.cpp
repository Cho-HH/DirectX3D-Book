		...
		UINT mipFilter)
{
	...
	{
		HR(CreateWICTextureFromFileEx(device, filenames[i].c_str(), 0, D3D11_USAGE_STAGING, 0,
			D3D11_CPU_ACCESS_WRITE | D3D11_CPU_ACCESS_READ, 0,
			WIC_LOADER_DEFAULT, (ID3D11Resource**)&srcTex[i], 0));
...
void ExtractFrustumPlanes(XMFLOAT4 planes[6], CXMMATRIX Matrix)
{
	XMFLOAT4X4 M;
