...
bool TextureHill::Init()
{
	...
	wchar_t Boxfilename[_MAX_EXT] = L"Textures/WireFence.dds";

	_wsplitpath_s(Boxfilename, drive, dir, name, ext);
	CreateShaderResourseViewFromFile(m_spDevice.Get(), Boxfilename, ext, m_BoxMapSRV.GetAddressOf());
...
void TextureHill::RenderFrame()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		m_spDeviceCon->RSSetState(RenderState::NoCullRS.Get());
		boxTech->GetPassByIndex(p)->Apply(0, m_spDeviceCon.Get());
		m_spDeviceCon->DrawIndexed(36, 0, 0);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		landAndWavesTech->GetPassByIndex(p)->Apply(0, m_spDeviceCon.Get());
...
void TextureHill::BuildCrateGeometryBuffers()
{
	...
	m_spDevice->CreateBuffer(&vbd, &vinitData, m_BoxVB.GetAddressOf());
	...
	m_spDevice->CreateBuffer(&ibd, &iinitData, m_BoxIB.GetAddressOf());
}
